---
title: "Concurrency Categorization (2015)"
summary: "(2015) If you agree with my assumptions that I’ll outline in the post, I believe that at its core, there are six dimensions to consider when evaluating a method for performing and handling asynchronous work. In this post, I provide a survey of concurrency primitives in the mobile app space categorized along these axes."
image: /static/posts/concurrency-categorization/title.png
date: "2020-12-31"
---

## Preface (2020)

Despite being written far enough ago that many of the concrete examples are irrelevant, I still find myself <Footnote alt={<span>You'd be surprised at how often I talk about this.</span>}>constantly sharing my concurrency categorization survey post with colleagues and friends whenever we discuss tradeoffs with different concurrency primitives</Footnote>. Since my old website has bit-rot, I figured I should bring this post here.

Note also that the content here is scoped to my experience at the time, an engineer who professionally had exclusively developed mobile apps. At this time, Apple had only just released Swift, and as far as I'm aware my organization was the only venture-backed company with any significant Kotlin in production. As such, many of my peers in the mobile space were not comfortable with functional idioms such as using `map` and `reduce` rather than for-loops. These days if I were to write a post like this I would include [coroutine](https://en.wikipedia.org/wiki/Coroutine)s alongside callbacks in the general section. I would also include [Combine](https://developer.apple.com/documentation/combine) and [Loom](https://openjdk.java.net/projects/loom/) under the mobile-app umbrella. Given that I now have worked with Rust and OCaml a bit, I would also include context on OCaml primitives like [Async](https://opensource.janestreet.com/async/), [Lwt](http://ocsigen.org/lwt), and even a discussion on [multicore's algebraic effects](https://kcsrk.info/slides/handlers_edinburgh.pdf), and Rust's [futures](https://docs.rs/futures/0.3.8/futures/).

As always, feel free to send me a tweet with any feedback, corrections, or comments to [@bkase_](https://twitter.com/bkase_).
...
(`+`) over the integers and (`x`) over the integers takes two integers and returns an integer. Thus both are composable operators (moreover they are monoids &mdash; Google it).
...
We also have parallel composition &mdash; if I can asynchronously get a name, and separately I’ve defined how to asynchronously get an age, I should be able to have one entity that can asynchronously get both a name and an age.
...
Mutable state makes code harder to reason over. Non-local mutable state is even worse. This is software engineering 101: Use functions, avoid globals. Some concurrency primitives are inherently unscoped &mdash; in other words, concurrent work can be manipulated and listened to from anywhere in your program. This is a global. This is not good.

## Learning Difficulty

This is a bit subjective &mdash; some primitives are harder to learn than others. Given the time constraints of your project, you may choose to use an easier-to-learn primitive that may not fit your needs as well in order to onboard your team faster.
...
* _Easy to Learn_: Good documentation.
...
* _Effectful_: The bus can produce messages &mdash; in order to listen you must attach callbacks.
...
This post should give you some idea of how to categorize your concurrency primitives if I did not cover it myself, and should help you analyze other concurrency options for your platform. Your choice of concurrency primitive matters: In a modern application, you will be using this primitive all over your codebase &mdash; utilizing the right concurrency primitive will let you and your team achieve both a maintainable and performant application.
